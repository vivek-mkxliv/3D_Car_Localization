...
void print_r599_detections(FILE *fp, char *id, box *boxes, float **probs, int total, int classes, int w, int h, float set_thresh)
{
	int i, j;
	for (i = 0; i < total; ++i) {
		float xmin = boxes[i].x - boxes[i].w / 2.;
		...
		for (j = 0; j < classes; ++j) {
			int classid = j;
			float conf = probs[i][classid] * 100.0;
			if (probs[i][classid] >= set_thresh) fprintf(fp, "%s, %d, %d, %d, %d, %d, %f\n", id, classid,
				c_x, c_y, obj_w, obj_h, conf);
		}
	}
}

void validate_detector(char *datacfg, char *cfgfile, char *weightfile, float set_thresh)
{
    int j;
    list *options = read_data_cfg(datacfg);
    ...
    if(0==strcmp(type, "coco")){
        ...
        imagenet = 1;
        classes = 200;
	} else if (0 == strcmp(type, "r599")) {
		snprintf(buff, 1024, "%s/r599-detection.txt", prefix);
		fp = fopen(buff, "w");
		r599 = 1;
		classes = l.classes;
	} else {
    ...
    for(i = nthreads; i < m+nthreads; i += nthreads){
        ...
        for(t = 0; t < nthreads && i+t-nthreads < m; ++t){
            ...
            if (coco){
				...
				print_r599_detections(fp, path, boxes, probs, l.w*l.h*l.n, classes, w, h, set_thresh);
			} else {
...
void run_detector(int argc, char **argv)
{
    ...
    else if(0==strcmp(argv[2], "valid")) validate_detector(datacfg, cfg, weights, thresh);
